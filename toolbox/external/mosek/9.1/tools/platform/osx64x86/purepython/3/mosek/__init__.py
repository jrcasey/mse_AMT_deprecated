...
__library__,__dlldir__ = loadmosek("mosekxx9_1")
...
class Task:
  ...
  def getclist(self,subj_,c_):
    ...
    if c_ is not None and len(c_) != (num_):
      raise ValueError("Array argument c is not long enough")
    if isinstance(c_,numpy.ndarray) and not c_.flags.writeable:
      raise ValueError("Argument c must be writable")
    if c_ is None:
  ...
  def generatevarnames(self,subj_,fmt_,dims_,sp_):
    ...
    if dims_ is None:
      ...
      _dims_tmp = None
    elif isinstance(dims_, numpy.ndarray) and dims_.dtype is numpy.dtype(numpy.int32) and dims_.flags.contiguous:
      _dims_copyarray = False
      _dims_tmp = ctypes.cast(dims_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int32))
    else:
      _dims_copyarray = True
      _dims_tmp_arr = numpy.array(dims_,numpy.int32)
      _dims_tmp = ctypes.cast(_dims_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int32))
        
    if sp_ is not None and len(sp_) != (num_):
      raise ValueError("Array argument sp is not long enough")
    if sp_ is None:
      _sp_copyarray = False
      _sp_tmp = None
    elif isinstance(sp_, numpy.ndarray) and sp_.dtype is numpy.dtype(numpy.int64) and sp_.flags.contiguous:
      _sp_copyarray = False
      _sp_tmp = ctypes.cast(sp_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _sp_copyarray = True
      _sp_tmp_arr = numpy.array(sp_,numpy.int64)
      _sp_tmp = ctypes.cast(_sp_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
  ...
  def getbarxslice(self,whichsol_,first_,last_,slicesize_,barxslice_):
    ...
    """
    if barxslice_ is not None and len(barxslice_) != (slicesize_):
      raise ValueError("Array argument barxslice is not long enough")
    ...
    if barxslice_ is None:
      ...
      _barxslice_tmp = None
    elif isinstance(barxslice_, numpy.ndarray) and barxslice_.dtype is numpy.dtype(numpy.float64) and barxslice_.flags.contiguous:
      _barxslice_copyarray = False
      _barxslice_tmp = ctypes.cast(barxslice_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_double))
    else:
      _barxslice_copyarray = True
      _barxslice_tmp_arr = numpy.array(barxslice_,numpy.float64)
      _barxslice_tmp = ctypes.cast(_barxslice_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_double))
  ...
  def getbarsslice(self,whichsol_,first_,last_,slicesize_,barsslice_):
    ...
    """
    if barsslice_ is not None and len(barsslice_) != (slicesize_):
      raise ValueError("Array argument barsslice is not long enough")
    ...
    if barsslice_ is None:
      ...
      _barsslice_tmp = None
    elif isinstance(barsslice_, numpy.ndarray) and barsslice_.dtype is numpy.dtype(numpy.float64) and barsslice_.flags.contiguous:
      _barsslice_copyarray = False
      _barsslice_tmp = ctypes.cast(barsslice_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_double))
    else:
      _barsslice_copyarray = True
      _barsslice_tmp_arr = numpy.array(barsslice_,numpy.float64)
      _barsslice_tmp = ctypes.cast(_barsslice_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_double))
  ...
  def getatruncatetol(self,tolzero_):
    ...
    """
    if tolzero_ is not None and len(tolzero_) != 1:
      raise ValueError("Array argument tolzero is not long enough")
    ...
    if tolzero_ is None:
      ...
      _tolzero_tmp = None
    elif isinstance(tolzero_, numpy.ndarray) and tolzero_.dtype is numpy.dtype(numpy.float64) and tolzero_.flags.contiguous:
      _tolzero_copyarray = False
      _tolzero_tmp = ctypes.cast(tolzero_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_double))
    else:
      _tolzero_copyarray = True
      _tolzero_tmp_arr = numpy.array(tolzero_,numpy.float64)
      _tolzero_tmp = ctypes.cast(_tolzero_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_double))
  ...
  def putbaraijlist(self,subi_,subj_,alphaptrb_,alphaptre_,matidx_,weights_):
    ...
    if alphaptrb_ is None:
      ...
      _alphaptrb_tmp = None
    elif isinstance(alphaptrb_, numpy.ndarray) and alphaptrb_.dtype is numpy.dtype(numpy.int64) and alphaptrb_.flags.contiguous:
      _alphaptrb_copyarray = False
      _alphaptrb_tmp = ctypes.cast(alphaptrb_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _alphaptrb_copyarray = True
      _alphaptrb_tmp_arr = numpy.array(alphaptrb_,numpy.int64)
      _alphaptrb_tmp = ctypes.cast(_alphaptrb_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    ...
    if alphaptre_ is None:
      ...
      _alphaptre_tmp = None
    elif isinstance(alphaptre_, numpy.ndarray) and alphaptre_.dtype is numpy.dtype(numpy.int64) and alphaptre_.flags.contiguous:
      _alphaptre_copyarray = False
      _alphaptre_tmp = ctypes.cast(alphaptre_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _alphaptre_copyarray = True
      _alphaptre_tmp_arr = numpy.array(alphaptre_,numpy.int64)
      _alphaptre_tmp = ctypes.cast(_alphaptre_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    ...
    if matidx_ is None:
      ...
      _matidx_tmp = None
    elif isinstance(matidx_, numpy.ndarray) and matidx_.dtype is numpy.dtype(numpy.int64) and matidx_.flags.contiguous:
      _matidx_copyarray = False
      _matidx_tmp = ctypes.cast(matidx_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _matidx_copyarray = True
      _matidx_tmp_arr = numpy.array(matidx_,numpy.int64)
      _matidx_tmp = ctypes.cast(_matidx_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
  ...
  def putbararowlist(self,subi_,ptrb_,ptre_,subj_,nummat_,matidx_,weights_):
    ...
    if nummat_ is not None and len(nummat_) != len((subj_)):
      raise ValueError("Array argument nummat is not long enough")
    if nummat_ is None:
    ...
    if nummat_ is None:
      ...
      _nummat_tmp = None
    elif isinstance(nummat_, numpy.ndarray) and nummat_.dtype is numpy.dtype(numpy.int64) and nummat_.flags.contiguous:
      _nummat_copyarray = False
      _nummat_tmp = ctypes.cast(nummat_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _nummat_copyarray = True
      _nummat_tmp_arr = numpy.array(nummat_,numpy.int64)
      _nummat_tmp = ctypes.cast(_nummat_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
        
    if matidx_ is not None and len(matidx_) != sum((nummat_)):
      raise ValueError("Array argument matidx is not long enough")
    if matidx_ is None:
      raise ValueError("Argument matidx cannot be None")
    if matidx_ is None:
      raise ValueError("Argument matidx may not be None")
    ...
    if weights_ is not None and len(weights_) != sum((nummat_)):
  ...
  def putconboundslice(self,first_,last_,bkc_,blc_,buc_):
    ...
    putconboundslice(self,first_,last_,bkc_,blc_,buc_)
      ...
      buc: array of double. Upper bounds for the constraints.
    """
    if bkc_ is not None and len(bkc_) != ((last_) - (first_)):
    ...
    if bkc_ is not None:
        ...
        _bkc_tmp = None
    if blc_ is not None and len(blc_) != ((last_) - (first_)):
    ...
    if buc_ is not None and len(buc_) != ((last_) - (first_)):
  ...
  def putvarboundslice(self,first_,last_,bkx_,blx_,bux_):
    ...
    putvarboundslice(self,first_,last_,bkx_,blx_,bux_)
      ...
      bux: array of double. Upper bounds for the variables.
    """
    if bkx_ is not None and len(bkx_) != ((last_) - (first_)):
    ...
    if bkx_ is not None:
        ...
        _bkx_tmp = None
    if blx_ is not None and len(blx_) != ((last_) - (first_)):
    ...
    if bux_ is not None and len(bux_) != ((last_) - (first_)):
  ...
  def appendsparsesymmatlist(self,dims_,nz_,subi_,subj_,valij_,idx_):
    ...
    if nz_ is None:
      _nz_copyarray = False
      _nz_tmp = None
    elif isinstance(nz_, numpy.ndarray) and nz_.dtype is numpy.dtype(numpy.int64) and nz_.flags.contiguous:
      _nz_copyarray = False
      _nz_tmp = ctypes.cast(nz_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _nz_copyarray = True
      _nz_tmp_arr = numpy.array(nz_,numpy.int64)
      _nz_tmp = ctypes.cast(_nz_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
        
    if subi_ is not None and len(subi_) != sum((nz_)):
      raise ValueError("Array argument subi is not long enough")
    if subi_ is None:
      raise ValueError("Argument subi cannot be None")
    if subi_ is None:
    ...
    if subj_ is not None and len(subj_) != sum((nz_)):
      raise ValueError("Array argument subj is not long enough")
    if subj_ is None:
      raise ValueError("Argument subj cannot be None")
    if subj_ is None:
    ...
    if valij_ is not None and len(valij_) != sum((nz_)):
    ...
    if idx_ is not None and len(idx_) != (num_):
      raise ValueError("Array argument idx is not long enough")
    ...
    if idx_ is None:
      ...
      _idx_tmp = None
    elif isinstance(idx_, numpy.ndarray) and idx_.dtype is numpy.dtype(numpy.int64) and idx_.flags.contiguous:
      _idx_copyarray = False
      _idx_tmp = ctypes.cast(idx_.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
    else:
      _idx_copyarray = True
      _idx_tmp_arr = numpy.array(idx_,numpy.int64)
      _idx_tmp = ctypes.cast(_idx_tmp_arr.ctypes._as_parameter_,ctypes.POINTER(ctypes.c_int64))
