function [Solution] = Synthetic_Simulation(strName, station, depth, Gridding, FileNames, Options)

% Run pipeline using synthetic data, without changing the entire pipeline
% variable names etc. 
...
% messing around (delete this)
%stemp = 1e-6:4e-6:2e-5
%for o = 1:numel(stemp)
%TpDat.S(1) = stemp(o);
...
% Solve LP
tic
%[n_opt(o,:),fval(o),exitflag,output] = fmincon(TpOptLP);
%[n_opt,fval,exitflag,output] = fmincon(TpOptLP);
...
r_opt = StrMod3.rInit;
%% MM Acclimation and Photoacclimation (comment out for no light)
...
% Comment out if natural temperature
TCorr = 1;
